...
const OUTSIDE_NODE_IDS = new Set([
  'room_gf_125', // Bus Bay
...
function anchorToCorridorLike(nodeId){
  ...
  if (OUTSIDE_NODE_IDS.has(nodeId)) {
    ...
    const nearestCorridor = nearestNodeOfType(nodeId, new Set(['corridor']));
    if (nearestEntrance && (!nearestCorridor || nearestEntrance.d <= nearestCorridor.d)) return nearestEntrance.id;
    if (nearestCorridor) return nearestCorridor.id;
  }
...
function drawRoute(pathObj){
  ...
  // markers for start and end (drawn at original positions if available)
  const s = getNode(pathObj.originalStart || pathObj.start);
  const e = getNode(pathObj.originalEnd || pathObj.end);
  if (s){
